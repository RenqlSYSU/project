...
PROGRAM SPECTRAL
  ...
  REAL(KIND=8), DIMENSION(:,:),   ALLOCATABLE :: x,f,ft,df
  COMPLEX(KIND=8), DIMENSION(:,:),ALLOCATABLE :: t
  ...
  OPEN(20,FILE='derivative.dat',FORM='binary',ACCESS='DIRECT',RECL=8*3*N)
  WRITE(20,rec=1) x 
  WRITE(20,rec=2) f
  WRITE(20,rec=3) df  !real first derivative
  WRITE(20,rec=4) ft  !first derivative by fourier transform
  CLOSE(20)
...
CONTAINS
  SUBROUTINE INIT(n,x0,x1,x,f,df,mode)
    ...
    REAL(KIND=8),    DIMENSION(3,n) ::  x,f,df
    INTEGER i
    REAL dx
  ...
  SUBROUTINE FFT_DERIVATIVE(n,x0,x1,f,t,df)
    ...
    REAL(KIND=8),    INTENT(IN) :: x0,x1
    COMPLEX(KIND=8), DIMENSION(n), INTENT(INOUT):: t
    REAL k
  ...
  SUBROUTINE FFT_F(n,r,c)
    ...
    COMPLEX(KIND=8), DIMENSION(n), INTENT(OUT):: c
    INTEGER j,k
