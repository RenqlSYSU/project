...
def draw_relation(corr,prob,labelx,labely,figdir):
    ...
    axe.set_xticklabels(labelx,fontsize=smfont)
    axe.set_yticklabels(labely,fontsize=smfont)
    axe.tick_params(top=True, bottom=False, labeltop=True, labelbottom=False)
    axe.plot([1.5,1.5],[-0.5,(len(labely)-0.5)],color="k")
    axe.plot([3.5,3.5],[-0.5,(len(labely)-0.5)],color="k")

    #axe.spines[:].set_visible(False)
    # Loop over data dimensions and create text annotations.
    for i in range(len(labely)):
        for j in range(len(labelx)):
            text = axe.text(j,i,"%.2f"%corr[i,j], ha="center", va="center", color="k",fontsize=smfont)
    ...
    plt.colorbar(im, ax=axe, shrink=.8)
