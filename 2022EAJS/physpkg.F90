...
subroutine tphysac (ztodt,   cam_in,  &
    ...
    integer :: ii,ik                 ! column indices, time frame
   ...
   if (carma_do_aerosol) then
    ...
    if ((calday.ge.day_s).and.(calday.le.day_e)) then  
    ...
    if ( stat_lat >= lats .and. stat_lat <= latn .and. stat_lon >= lonl .and. stat_lon<= lonr) then
        ...
        if ( stat_lat >= (lats+buff) .and. stat_lat <= (latn-buff) ) then
            if ( stat_lon >= (lonl+buff) .and. stat_lon <= (lonr-buff) ) then 
                e = 1
            else
                e = exp ( -( abs( stat_lon - (lonl+lonr)/2 )- ((lonr-lonl)/2-buff) ))
            end if
        else
            if ( stat_lon >= (lonl+buff) .and. stat_lon <= (lonr-buff) ) then 
                e = exp ( -( abs( stat_lat - (lats+latn)/2 )- ((latn-lats)/2-buff) ))
            else
                e = exp ( -( sqrt( (stat_lon - (lonl+lonr)/2)**2 + (stat_lat - (lats+latn)/2)**2 ) - sqrt( ((latn-lats)/2-buff)**2 + ((lonr-lonl)/2-buff)**2 )))
            end if
        end if
        
        loop2:do ik = 1,pver
            if(state%pmid(ii,ik).ge.10000.0 .and. state%pmid(ii,ik).le.40000.0) then !unit is Pa
                ptend%u(ii,ik) = ptend%u(ii,ik)+e*G0*(urelax(ii,ik,lchnk)-state%u(ii,ik))
            end if
        end do loop2
        ...
        if ((calday-floor(calday))<0.01) then 
            ...
            (abs(stat_lat-((lats+latn)/2.0))<0.5)) then
                write(iulog,"(A28,F8.2,A5,F6.2,A5,F6.2,A15,F6.2,A5,F8.6,A15,F6.2)") "L_Zealot Mod Flag: Calday:",calday,&
                "Lat:",stat_lat,"Lon:",stat_lon," urelax(lv13)=", urelax(ii,13,lchnk),&
                "G0=",G0,"udiff(lv13)=",urelax(ii,13,lchnk)-state%u(ii,13)
                write(iulog,"(A32,F12.4,A10,F12.4,A8,I3,A8,I3,A10,F12.4,A10,F12.4,A10,F12.4)")  "L_Zealot Mod Flag: state_phis=",state%phis(ii),&
                "lev1=",state%pmid(ii,1), "nlev=",ik, "pver=",pver, "lev30=",state%pmid(ii,30), &
                "lev13=",state%pmid(ii,13), "lev20=",state%pmid(ii,20)
            end if
        end if
    end if
    end do loop1
