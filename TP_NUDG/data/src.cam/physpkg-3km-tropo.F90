...
subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_out )
    ...
    use comsrf,           only: urelax, vrelax, trelax
    !*** MOD END: Use module
    ...
    type(file_desc_t) ::  fh_urelax, fh_vrelax, fh_trelax
    !allocate(rfmask(pcols,begchunk:endchunk,12))   ! Rainfall Mask flag
    logical :: found=.false.
    integer :: nm   ! time frame controller 
    character(len=256) :: urelax_loc, vrelax_loc, trelax_loc   ! filepath of urelax file 
    ...
    urelax_loc="/WORK/sysu_hjkx_ys/renql/NUDG/F2000_CAM5/pre_ctrl/input2TP_NUDG/clim/F2000_CAM5.cam.h1.ESM.clim.U.nc"
    vrelax_loc="/WORK/sysu_hjkx_ys/renql/NUDG/F2000_CAM5/pre_ctrl/input2TP_NUDG/clim/F2000_CAM5.cam.h1.ESM.clim.V.nc"
    trelax_loc="/WORK/sysu_hjkx_ys/renql/NUDG/F2000_CAM5/pre_ctrl/input2TP_NUDG/clim/F2000_CAM5.cam.h1.ESM.clim.T.nc"
    call cam_pio_openfile(fh_urelax, urelax_loc, 0)
    call cam_pio_openfile(fh_vrelax, vrelax_loc, 0)
    call cam_pio_openfile(fh_trelax, trelax_loc, 0)
    do nm=1, 365 
        call infld('U', fh_urelax, 'lon', 'lev', 'lat', 1, pcols, 1, pver, begchunk, endchunk, &
             urelax(:,:,:,nm), found, grid_map='PHYS', timelevel=nm)
        
        call infld('V', fh_vrelax, 'lon', 'lev', 'lat', 1, pcols, 1, pver, begchunk, endchunk, &
             vrelax(:,:,:,nm), found, grid_map='PHYS', timelevel=nm)
        
        call infld('T', fh_trelax, 'lon', 'lev', 'lat', 1, pcols, 1, pver, begchunk, endchunk, &
             trelax(:,:,:,nm), found, grid_map='PHYS', timelevel=nm)
    end do
...
subroutine tphysac (ztodt,   cam_in,  &
    ...
    real(r8) :: calday,  G0, e ,day_s, day_e
    real(r8) :: stat_lat, stat_lon, lats, latn, lonl, lonr, buff
    integer :: ii,timeframe,ik                 ! column indices, time frame
   ...
   if (carma_do_aerosol) then
    ...
    !*** MOD START: Relaxing the U/V/T to the model state
    calday = get_curr_calday()
    G0=1.0/(86400*0.25)    ! How long to relax to the given field, 
    !Below to get out which month we are in
    
    lats = 20.0 ! include buffer zone
    latn = 45.0
    lonl = 65.0
    lonr = 110.0
    buff = 5.0
    day_s = 0.0   !121.0 !May 5
    day_e = 366.0 !244.0 !Sep 1

    if ((calday.ge.day_s).and.(calday.lt.day_e)) then  
    loop1:do ii = 1,ncol   ! cols in chunk 
        stat_lat=state%lat(ii)*180.0/3.1415926
        stat_lon=state%lon(ii)*180.0/3.1415926
    if ( stat_lat >= lats .and. stat_lat <= latn .and. stat_lon >= lonl .and. stat_lon<= lonr) then
        ! define the coefficient of buffer
        if ( state%phis(ii) >= 30000) then
            e = 1
        else
            e = exp ( -(abs( state%phis(ii) - 30000 )))
        end if

        timeframe=floor(calday)
        loop2:do ik = 1,pver
            if(state%pmid(ii,ik).gt.20000.0) then !unit is Pa
                ! Let's nudge the field!
                ptend%u(ii,ik) = ptend%u(ii,ik)+e*G0*(urelax(ii,ik,lchnk,timeframe)-state%u(ii,ik))
                ptend%v(ii,ik) = ptend%v(ii,ik)+e*G0*(vrelax(ii,ik,lchnk,timeframe)-state%v(ii,ik))
                ptend%s(ii,ik) = ptend%s(ii,ik)+e*G0*(trelax(ii,ik,lchnk,timeframe)-state%t(ii,ik))*cpair
        !    else
        !        exit loop2
            end if
        end do loop2
        ...
        if ((calday-floor(calday))<0.01) then 
            if ((abs(stat_lon-((lonl+lonr)/2.0))<0.5).and. &
            (abs(stat_lat-((lats+latn)/2.0))<0.5)) then
                write(iulog,"(A28,F8.2,A5,F6.2,A5,F6.2,A15,F6.2,A15,F6.2,A5,F8.6,A15,F6.2,A15,F6.2)")  "L_Zealot Mod Flag: Calday:",calday,&
                ...
                write(iulog,"(A28,F8.2,A5,F6.2,A5,F6.2,A15,F6.2,A15,F6.2,A5,F8.6,A15,F6.2,A15,F6.2)")  "L_Zealot Mod Flag: Calday:",calday,&
                "Lat:",stat_lat,"Lon:",stat_lon," urelax(lv20)=", urelax(ii,20,lchnk,timeframe)," vrelax(lv20)=", vrelax(ii,20,lchnk,timeframe),&
                "G0=",G0,"udiff(lv20)=",urelax(ii,20,lchnk,timeframe)-state%u(ii,20),"vdiff(lv20)=",vrelax(ii,20,lchnk,timeframe)-state%v(ii,20)
                write(iulog,"(A32,F12.4,A10,F12.4,A8,I3,A8,I3,A10,F12.4,A10,F12.4,A10,F12.4)")  "L_Zealot Mod Flag: state_phis=",state%phis(ii),&
                "lev1=",state%pmid(ii,1), "nlev=",ik, "pver=",pver, "lev30=",state%pmid(ii,30), "lev13=",state%pmid(ii,13), "lev20=",state%pmid(ii,20)
            end if
        end if
   ! else
   !     exit loop1
   ...
   !*** MOD END: Relaxing the U/V/T to the model state
