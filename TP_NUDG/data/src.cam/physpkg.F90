...
subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_out )
    ...
    urelax_loc="/users/yangsong3/L_Zealot/F/AMIP_C5PM/input_data2TP_NUDG_UVT/clim/AMIP_C5PM.cam.h1.ESM.clim.U.nc"
    vrelax_loc="/users/yangsong3/L_Zealot/F/AMIP_C5PM/input_data2TP_NUDG_UVT/clim/AMIP_C5PM.cam.h1.ESM.clim.V.nc"
    trelax_loc="/users/yangsong3/L_Zealot/F/AMIP_C5PM/input_data2TP_NUDG_UVT/clim/AMIP_C5PM.cam.h1.ESM.clim.T.nc"
...
subroutine tphysac (ztodt,   cam_in,  &
   ...
   if (carma_do_aerosol) then
    ...
    if ( stat_lat >= 20.0 .and. stat_lat <= 45.0 .and. stat_lon >= 65.0 .and. stat_lon<= 110.0) then
        ...
        if ( stat_lat >= 25.0 .and. stat_lat <= 40.0 ) then
            if ( stat_lon >= 70.0 .and. stat_lon <= 105.0 ) then 
                ...
                e = exp ( -( abs( stat_lon - 82.5 )- 12.5 ))
            end if
        else
            if ( stat_lon >= 70.0 .and. stat_lon <= 105.0 ) then 
                e = exp ( -( abs( stat_lat - 32.5 )- 7.5 ))
            else
                e = exp ( -( sqrt( (stat_lon - 82.5)**2 + (stat_lat - 32.5)**2 ) - sqrt( 12.5**2 + 7.5**2 )))
            end if
        end if

        timeframe=floor(calday)
        ! Let's nudge the field!
        ptend%u(ii,:) = ptend%u(ii,:)+e*G0*(urelax(ii,:,lchnk,timeframe)-state%u(ii,:))
        ptend%v(ii,:) = ptend%v(ii,:)+e*G0*(vrelax(ii,:,lchnk,timeframe)-state%v(ii,:))
        ptend%s(ii,:) = ptend%s(ii,:)+e*G0*(trelax(ii,:,lchnk,timeframe)-state%t(ii,:))*cpair

        ! Below to output debug info
        if ((calday-floor(calday))<0.01) then 
            ! Test
            if ((abs(stat_lon-85.0)<0.5).and. &
