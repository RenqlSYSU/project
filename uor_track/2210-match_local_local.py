...
def main_run():
    for nl1 in lev:
        for nl2 in lev:
            outfile = '%s/fftadd_match_%dlocal_%dlocal_%ddist'%(
                outdir,nl1,nl2,radiu2)
            match_local_remote('%s/fftadd_%d_1980-2020_%dlocal'%(path,nl1,radiu1),
                '%s/fftadd_%d_1980-2020_%dlocal'%(path,nl2,radiu1),radiu2,outfile)
...
def match_local_remote(filname1,filname2,dist,outfilename):
    ...
    while line:
        ...
        if term[0] == "TRACK_ID":
            ...
            if (filname1==filname2 and len(signal) >= 2) or (filname1!=filname2 and len(signal)>=1):
                tid.append(term[2])
...
def calc_month(outfile):
    ...
    for nl1 in range(len(lev)):
        for nl2 in range(len(lev)):
            filname = '%s/fftadd_match_%dlocal_%dlocal_%ddist'%(
