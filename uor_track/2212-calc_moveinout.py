...
def main_run():
    ...
    outfile = '%s/movein_moveout_month_%dcyclone_%drad.nc'%(
...
def calc_lysis_percent(outfile):
    ...
    var = np.zeros([len(lev),len(behv),nmonth], dtype=int)
    for nl in range(len(lev)):
        filname = '%s/ff_%d_1980-2020_%dtotal'%(path,lev[nl],radiu1)
        var[nl,:,:] = calc_season_cyclone(filname) 
        #var[nl,:,:] = calc_season_cyclone_lon(filname) 
...
def calc_season_cyclone_lon(filname):
    var = np.zeros([len(behv),nmonth], dtype=int ) # 4 or 12 
    ff = open(filname,"r") 
    line1 = ff.readline()
    line2 = ff.readline()
    line3 = ff.readline()
